...
type AcademicYearConfigurationDetailsProps = {
  rowId: number;
};

export const AcademicYearConfigurationDetails = ({
  rowId,
}: AcademicYearConfigurationDetailsProps) => {
  const row = useAcademicYearConfigurationStore((state) =>
  ...
  return (
    <div className="w-full max-w-220 space-y-6">
      <DashboardCard
        title={`Academic Year Configuration #${row.id}`}
        subtitle="Review the stored values for this academic year configuration record."
        action={
          <div className="flex flex-wrap items-center gap-3">
            ...
            <Link
              href={`/academic-year-configuration/${row.id}/edit`}
...
export default AcademicYearConfigurationDetails;
